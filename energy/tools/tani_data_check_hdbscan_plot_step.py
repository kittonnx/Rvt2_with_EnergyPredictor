...
OUTPUT_NPZ_PATH = 'fused711_hdbscan-0.001-sample-5-MIN-20-step.npz' # 保存する新しいファイル名
...
MIN_SAMPLE = 5
ABSOLUTE_MIN_SIZE = 20    # これより小さい集団はクラスタと認めない
...
def plot_step_facets(X_features_3d_all, cluster_labels_all, task_name, output_dir):
    """
    HDBSCANの結果を、ステップごとにファセットプロットして可視化する
    (各ステップの「距離 vs エネルギー」をクラスタ別に色分け)
    
    Args:
        X_features_3d_all (np.array): タスク全体の (移動距離, エネルギー, ステップ番号)
        cluster_labels_all (np.array): タスク全体の (ステップごとに計算された) ラベル
        task_name (str): タスク名
        output_dir (str): 保存先
    """
    ...
    # ★注意: cluster_labels_all はステップごとに独立している
    # (Step0のCluster0 と Step1のCluster0 は無関係)
    # したがって、色もステップごとに独立して割り当てる
    ...
    for i, step in enumerate(unique_steps):
        ...
        if len(step_features_3d) == 0:
            ax.set_title(f'Step {step} (N=0)')
        ...
        for label in step_unique_labels:
            ...
            data_2d = step_features_3d[label_mask][:, :2] # 距離とエネルギー
            count = len(data_2d)
            
            if label == -1:
                color = 'black' # ★あなたの設定 (black) に合わせる
                label_text = f'Outlier (-1) (N={count})'
                marker = 'x' # 外れ値はx
            else:
        ...
        ax.set_title(f'Step {step} (Total N={len(step_features_3d)})')
        ax.set_xlabel(xlabel)
        ax.set_ylabel(ylabel)
    ...
    fig.suptitle(f'Per-Step Cluster Analysis - Task: {task_name}', fontsize=16, y=1.03)
    fig.tight_layout()
    ...
    for task_name in all_task_names:
        ...
        for step in unique_steps_in_task:
            ...
            if initial_count < (ABSOLUTE_MIN_SIZE * 2):
                print(f"  [Task: {step_task_name}] ... データ数 ({initial_count}) が少なすぎるため、ハズレ地とする。")
                core_mask = np.full(initial_count, False) # ★あなたのコードのロジックを維持
