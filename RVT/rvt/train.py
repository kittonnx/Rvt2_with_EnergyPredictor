...
def dump_log(exp_cfg, mvt_cfg, cmd_args, log_dir, agent):
    ...
    with open(f"{log_dir}/energy_config.yaml", "w") as yaml_file:
        yaml.dump(agent.energy_config, yaml_file)
...
def experiment(rank, cmd_args, devices, port):
    ...
    EARLY_STOP = {
        "metric": "total_loss",  # train() が返す dict のキー
        "patience": 5,          # 連続未改善で停止
        "min_delta": 3e-3,        # 改善とみなす最小差
    }
    ...
    # MVTやbitsandbytesがCUDAを初期化する *前* に 'fork' を強制する
    if exp_cfg.num_workers > 0:
        print("Pre-warming persistent DataLoader workers...")
        _data_iter = iter(train_dataset)
        _ = next(_data_iter) # 最初のバッチを強制的にロードさせてワーカーを起動
        print("DataLoader workers are ready.")
    ...
    if rank == 0:
        ...
        dump_log(exp_cfg, mvt_cfg, cmd_args, log_dir, agent)
        ...
        wandb_cfg = {
            "exp": dict(exp_cfg),
            "mvt": dict(mvt_cfg) if exp_cfg.agent == "our" else {},
            "args": cmd_args.__dict__,
            "world_size": len(devices),
        }
        wandb.init(
            # project="TF_finetune",                  # 任意のプロジェクト名
            project="TF_add_exp",                  # 任意のプロジェクト名
            name="test-otani-0218",            # 実験名
            config=wandb_cfg,
    ...
    while True:
        ...
        if rank == 0:
            ...
            loss_keys = ["total_loss","trans_loss","rot_loss_x","rot_loss_y","rot_loss_z",
                        "grip_loss","collision_loss","energy_loss"]
            metrics = {f"train/{k}": out[k] for k in loss_keys if k in out}
            metrics["train/rot_loss_sum"] = sum(out.get(k, 0.0) for k in ["rot_loss_x","rot_loss_y","rot_loss_z"])
            metrics["train/rgc_loss_sum"] = out.get("grip_loss",0.0) + out.get("collision_loss",0.0)
            metrics["train/lr"] = out.get("lr", 0.0)

            wandb.log(metrics, step=i) 

            metric = out.get(EARLY_STOP["metric"], None)
            if metric is not None:
                improved = (best - metric) > EARLY_STOP["min_delta"]
                if improved:
                    best = metric
                    no_improve = 0
                    save_agent(agent, f"{log_dir}/model_best.pth", i)
                else:
                    no_improve += 1
                    stop_flag = int(no_improve >= EARLY_STOP["patience"])

                wandb.log({
                    "early_stop/metric": metric,
                    "early_stop/best": best,
                    "early_stop/no_improve": no_improve,
                    "early_stop/stop": stop_flag,
                })

        # 全rank同期
        stop_tensor = torch.tensor([stop_flag], device=rank_device)
