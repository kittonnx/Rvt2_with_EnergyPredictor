...
def extract_obs(obs: Observation,
    ...
    if joint_vel is None or joint_pos is None or grip_pose is None:
        print("tanipon here")
    ...
    if (joint_pos is not None) and (joint_vel is not None) and (grip_pose is not None):
        energy_rs = np.concatenate([joint_pos, joint_vel, grip_pose]).astype(np.float32)  # 7+7+7 = 21
        obs_dict['energy_robot_state'] = energy_rs
