...
def get_robot_state(eval_env):
    arm = eval_env._task._scene.robot.arm

    joint_angles = arm.get_joint_positions()
    joint_velocities = arm.get_joint_velocities()

    tip = arm.get_tip()
    ee_pose = tip.get_pose()
...
def eval(
    ...
    if logging:
        ...
        tani_csv_fp = None
        tani_csv_writer = None
        # verbose_log.csv を log_dir 配下に作成
        csv_path = os.path.join(log_dir or ".", "tani_log.csv")
        tani_csv_fp = open(csv_path, mode="w", newline="", encoding="utf-8")
        tani_csv_writer = csv.DictWriter(tani_csv_fp, fieldnames=[
            "task_name",
            "episode",
            "reward",
            "episode_length",
        ...
        tani_csv_writer.writeheader()
        tani_csv_fp.close()

        energy_log_path = os.path.join(log_dir, "energy_dataset.npz")
        energy_datas = []
    ...
    for task_id in range(num_tasks):
        ...
        for ep in range(start_episode, start_episode + eval_episodes):
            ...
            prev_total_energy = 0
            input_data = {
                'robot_state': get_robot_state(eval_env)
            } 

            try:
                for step, replay_transition in enumerate(generator, 1):
                    # energy抽出
                    current_total_energy = eval_env.get_total_energy()
                    energy = current_total_energy - prev_total_energy

                    input_data['task_name'] = task_name
                    input_data['episode'] = task_ep
                    input_data['step'] = step

                    input_data['predicted_action'] = agent.last_action_dict
                    input_data['energy'] = energy
                    input_data['soft_prediction'] = agent.last_soft_prediction
                    input_data['lang_goal'] = eval_env._lang_goal
                    
                    input_data['rvt_fused_feat'] = agent.last_fused_feat.cpu().numpy().squeeze(0)

                    # mvt_single = agent._network.mvt1
                    # input_data['rvt_proprio_feat'] = mvt_single.proprio_feat.detach().cpu().numpy()
                    # input_data['rvt_lang_feat'] = mvt_single.lang_feat.detach().cpu().numpy()


                    episode_data_buffer.append(input_data)
            ...
            # tani
            energy = eval_env.get_total_energy()

            if reward == 100.0:
                energy_datas.extend(episode_data_buffer)
                task_ep += 1

            if verbose:
                tani_csv_fp = open(csv_path, mode="a", newline="", encoding="utf-8")
                tani_csv_writer = csv.DictWriter(tani_csv_fp, fieldnames=[
                    "task_name",
                    "episode",
                    "reward",
                ...
                tani_result = {"task_name": task_name, "episode": ep, "reward": reward, "episode_length": len(episode_rollout), "lang_goal": lang_goal, "energy": energy}
                tani_csv_writer.writerow(tani_result)
                tani_csv_fp.close()

            if verbose:
                print(
                    f"Evaluating {task_name} | Episode {ep} | Score: {reward} | Episode Length: {len(episode_rollout)} | Lang Goal: {lang_goal} | energy: {energy}"
                )

        if len(energy_datas) > 0:
            print("energydatas 出力：", energy_log_path)
            predicted_actions = [d['predicted_action'] for d in energy_datas]
            soft_predictions = [d['soft_prediction'] for d in energy_datas]
            
            np.savez_compressed(
                energy_log_path,
                task_name = np.array([d['task_name'] for d in energy_datas]),
                episode = np.array([d['episode'] for d in energy_datas]),
                step = np.array([d['step'] for d in energy_datas]),
                robot_state = np.array([d['robot_state'] for d in energy_datas]),
                predicted_position = np.array([d['position'] for d in predicted_actions]),
                predicted_rotation_quat = np.array([d['rotation_quat'] for d in predicted_actions]),
                predicted_gripper_state = np.array([d['gripper_state'] for d in predicted_actions]),
                predicted_collision_state = np.array([d['collision_state'] for d in predicted_actions]),

                # ソフトな予測結果 (softmaxによる)
                soft_predicted_position = np.array([d['position'] for d in soft_predictions]),
                soft_predicted_rotation_quat = np.array([d['rotation_quat'] for d in soft_predictions]),
                soft_predicted_gripper_state = np.array([d['gripper_state'] for d in soft_predictions]),
                soft_predicted_collision_state = np.array([d['collision_state'] for d in soft_predictions]),
                # 言語ゴール
                lang_goal = np.array([d['lang_goal'] for d in energy_datas]),


                rvt_fused_feat = np.array([d['rvt_fused_feat'] for d in energy_datas]),
                # rvt_proprio_feat = np.array([d['rvt_proprio_feat'].squeeze() for d in energy_datas]),
                # rvt_lang_feat = np.array([d['rvt_lang_feat'].squeeze() for d in energy_datas]),
                energy = np.array([d['energy'] for d in energy_datas])
            )
        ...
        if save_video:
            video_image_folder = os.path.join(log_dir, "tmp_video_image")
