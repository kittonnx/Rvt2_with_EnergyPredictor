...
def create_replay(
    ...
    observation_elements.append(
        ObservationElement("energy_robot_state", (21,), np.float32)
    )
...
def _add_keypoints_to_replay(
    ...
    for k in range(
        ...
        ers = np.asarray(obs_dict['energy_robot_state'], dtype=np.float32)
        assert ers.ndim == 1 and ers.size == 21, f"energy_robot_state shape={ers.shape}"
        others['energy_robot_state'] = ers
    ...
    ers = np.asarray(obs_dict_tp1['energy_robot_state'], dtype=np.float32)
    assert ers.ndim == 1 and ers.size == 21, f"energy_robot_state shape={ers.shape}"
    obs_dict_tp1['energy_robot_state'] = ers
